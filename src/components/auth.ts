...
const minLength = (length: number): ValidationFn => (value: string) =>
  value.length >= length ? null : `${length}자 이상이어야 합니다`;

const maxLength = (length: number): ValidationFn => (value: string) =>
...
export const validateUserId = (userId: string): string | null => {
  return (
    minLength(1)(userId) ||
    maxLength(10)(userId) ||
    containsDigit(userId) ||
    containsLowercase(userId) ||
    containsUppercase(userId)
  );
};
